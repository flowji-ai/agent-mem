...
Date Updated: 2026-03-23T22:55:21+11:00
status: completed
...
**Tests FIRST (Red phase):** Write `tests/sqlite/schema/summary-columns.test.ts` before creating the schema module:
- `SUMMARY_CONTENT_COLUMNS` includes `['request', 'investigated', 'learned', 'completed', 'next_steps', 'files_read', 'files_edited', 'notes']`
- `SUMMARY_META_COLUMNS` includes `['id', 'memory_session_id', 'project', 'prompt_number', 'discovery_tokens', 'created_at', 'created_at_epoch']`
- `SUMMARY_ALL_COLUMNS` is meta + content combined, no duplicates
- `SUMMARY_FTS_COLUMNS` is `['request', 'investigated', 'learned', 'completed', 'next_steps', 'notes']`
- `SUMMARY_INSERT_COLUMNS` is `['memory_session_id', 'project', 'request', 'investigated', 'learned', 'completed', 'next_steps', 'files_read', 'files_edited', 'notes', 'prompt_number', 'discovery_tokens', 'created_at', 'created_at_epoch']`
- `summarySelectCols()` produces correct SQL string
- `summaryInsertPlaceholders()` produces correct number of `?` placeholders
...
**Verify tests FAIL:** `bun test` — tests should fail because the schema module doesn't exist yet.

**Implementation (Green phase):** Create the schema module:
- `src/services/sqlite/schema/summary-columns.ts` — single source of truth
- `src/services/sqlite/schema/observation-columns.ts` — same for observations
- `src/services/sqlite/schema/index.ts` — barrel export

**Verify tests PASS:** `bun test`, `tsc --noEmit`
**Commit:** `refactor: add central schema column constants for summaries and observations`

### Step 2: Consolidate type definitions

This step is a pure refactor — no new behaviour, so existing tests ARE the red-green gate. The "Red phase" is verifying compilation breaks when you remove duplicates; the "Green phase" is fixing the imports so everything compiles again.
...
**Verify:** `tsc --noEmit` (compilation errors = imports not yet updated — fix them all), then `bun test` (all existing tests must pass with new import paths)
...
**Tests FIRST (Red phase):** Add regression tests to `tests/sqlite/schema/summary-columns.test.ts`:
- For each SELECT query being replaced, capture the original hardcoded SQL string as the expected value
- Test that `summarySelectCols(SUBSET)` produces an identical string for each query's column subset
- These tests should PASS immediately (they test the helper against known strings, not the replacement)

**Implementation (Green phase):** Replace inline column strings with `summarySelectCols(...)` in:
...
**Verify:** `bun test` (regression tests confirm generated SQL matches originals), worker starts and serves viewer at `localhost:37777`
**Commit:** `refactor: replace hardcoded SELECT column lists with central constants`

### Step 4: Replace hardcoded INSERT column lists

**Tests FIRST (Red phase):** Add INSERT roundtrip test to `tests/sqlite/schema/summary-columns.test.ts`:
- Create a `:memory:` DB with the summary table
- INSERT a row using `SUMMARY_INSERT_COLUMNS` and `summaryInsertPlaceholders()` with known test values (e.g. request='test_request', investigated='test_investigated', etc.)
- Read it back with `SELECT *`
- Assert every field has the expected value in the expected column — this catches parameter order mismatches which silently corrupt data
- This test should PASS immediately (it tests the helper, not the replacement)

**Implementation (Green phase):** Replace inline INSERT SQL in:
...
**Important:** After each replacement, verify the parameter array passed to `db.run()` is in the same order as `SUMMARY_INSERT_COLUMNS`. A mismatch silently corrupts data.
