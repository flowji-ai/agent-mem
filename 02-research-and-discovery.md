...
**Status:** Answered in `04-research-findings.md`. The Stop hook fires after every Claude response — not at session end. This is why we call these **snapshots**, not session summaries.
...
**Status:** Answered. Current fields: `request`, `investigated`, `learned`, `completed`, `next_steps`, `notes`. See `05-phase-one-brief.md` for full replacement schema.
...
**Status:** Answered in `04-research-findings.md`. The secondary AI agent (Qwen in Ostii's setup) receives `buildSummaryPrompt()` from `src/sdk/prompts.ts` and returns XML. Summary is generated from Claude's last assistant message only — not the full transcript.
...
**Status:** Answered. SQLite at `~/.claude-mem/claude-mem.db` for structured data. ChromaDB at `~/.claude-mem/vector-db/` for semantic search (optional — SQLite FTS5 fallback available). See `04-research-findings.md`.
...
**Status:** Resolved. `hidden_fields TEXT` column added in Phase 1 migration (JSON array of unchecked field names). Injection query filters excluded fields. UI shows them greyed out. See `05-phase-one-brief.md` Task 5.
...
**Status:** Complete. See `07-research-effective-agent-memory.md`.
...
**Status:** Provisional design complete — see `05-phase-one-brief.md` Task 4. Implementation approach (prompt-level guidance vs transcript-level scanning) still to be confirmed. Transcript parser already exists at `src/services/transcripts/` — check if it can be leveraged before building anything new.
